...
function curl_handle($url, $comments)
{
    $curl_handle = curl_init();
    curl_setopt($curl_handle, CURLOPT_URL, 'https://graph.facebook.com/' . $comments . '?ids=' . $url);
    curl_setopt($curl_handle, CURLOPT_CONNECTTIMEOUT, 2);
    curl_setopt($curl_handle, CURLOPT_RETURNTRANSFER, 1);
    curl_setopt($curl_handle, CURLOPT_USERAGENT, 'Facebook Comments Sync');
...
function fbcsf_runfbcs()
{
    ...
    if ($stop < $postcount) {
        for ($i = $start; $i <= $stop; $i++) {
            fbcsf_syncCommentCounts($postsID[$i]);
            fbcsf_impComments($postsID[$i]);
        }
    };
    echo $stop;
